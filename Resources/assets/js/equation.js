...
function CountVar( s ) {
	...
	for (var i = 0; i < s.length; ++i) {
		if (!(s[i] in chars)) { //eğer girdinin i. karakteri chars dizisinde yoksa 
			chars[s[i]] = 1;
...
document.getElementById('equation').addEventListener('change', function() {
	...
	if (isNaN(this.value) && (this.value).match(/[0-9]/)==null ){ //girilen değer sayı değilse işleme al
		...
		console.log(strlower);
		switch(varNum)  //harf sayısını kontrol ediyor 
		{
			...
			default: //3 veya 4 değilse 
				if (varNum < 3) {   //3'ten küçükse iki değişkeni seçiyor

					document.getElementById('TwoVariableRB').click();
					replaceVar(strlower);

				}else if (varNum > 4){  //4'ten büyükse yanlış giriş uyarısı veriyor
					alert("Invalid input");
				};
		}
